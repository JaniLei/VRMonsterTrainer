        ...
        set
        {
            ...
            else
            {
                if (state == RagdollState.ragdolled)
                {
                    ...
                    ragdolledHeadPosition = head.position;// anim.GetBoneTransform(HumanBodyBones.Head).position;
                    ragdolledHipPosition = hip.position;// anim.GetBoneTransform(HumanBodyBones.Hips).position;
    ...
    void LateUpdate()
    {
        ...
        if (state == RagdollState.blendToAnim)
        {
            if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime)
            {
                ...
                Vector3 animatedToRagdolled = ragdolledHipPosition - hip.position;// anim.GetBoneTransform(HumanBodyBones.Hips).position;
            ...
            foreach (BodyPart b in bodyParts)
            {
                ...
                { //this if is to prevent us from modifying the root of the character, only the actual body parts
                  //position is only interpolated for the hips
                    if (b.transform == hip/*anim.GetBoneTransform(HumanBodyBones.Hips)*/)
